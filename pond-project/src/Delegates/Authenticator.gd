...
# recover it without needing the authentication server, unless force_new_session is true,
# in which case, the session will be created through authentication
# Returns OK or a nakama error code. Stores error messages in `ServerConnection.error_message`
func login_async(email: String, password: String, force_new_session: bool = false) -> int:
	...
	# If previous session is unavailable, invalid, or expired, starts a new session
	var new_session: NakamaSession = yield(
		_client.authenticate_email_async(email, password, email, false), "completed"
	)
	var parsed_result := _exception_handler.parse_exception(new_session)
	if parsed_result == OK:
		session = new_session
...
class SessionFileWorker:
	...
	static func recover_session_token(email: String, password: String) -> String:
		...
		if error == OK:
			# get_line reads in the written order
