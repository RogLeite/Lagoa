...
function M.add_reservation(worlds, world_id, user_id)
...
function M.add_presence(worlds, world_id, user_presence)
    ...
    if not M.is_present(worlds, world_id, id) then
...
function M.remove_presence(worlds, world_id, user_presence)
    local id = user_presence.user_id
    if M.is_present(worlds, world_id, id) then
        M.get_world(worlds, world_id).player_presences[id] = nil
    end
end
